...
describe('useApiRouteFetcher with real $fetch requests', () => {
  ...
  it('should send POST request with JSON Content-Type', async () => {
    const { post, raw } = useApiRouteFetcher();
...
describe('merge options', () => {
  const op1 = {
    onRequest: attachId('op1Req1', () => { logger.debug('onRequest 1'); }),
    onRequestError: attachId('op1Req2', () => { logger.debug('onRequestError 1'); }),
    onResponse: attachId('op1Res1', () => { logger.debug('onResponse 1'); }),
    onResponseError: attachId('op1Res2', () => { logger.debug('onResponseError 1'); }),
  ...
  it('returns ops if op is null or undefined', () => {
    const { opt } = useApiRouteFetcher(op1);
  ...
  it('handles identify merging', () => {
    ...
    expect(opt(op)).toEqual({
      ...
      onResponse: [op2.onResponse],
      onResponseError: [op2.onResponseError],
    });
  });
});
