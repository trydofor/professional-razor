...
export function useApiRoute() {
  ...
  return {
    ...
    get: <Data>(uri: string, query?: Record<string, unknown>) => $fetch<DataResult<Data>>(
      prefix + uri,
      {
        method: 'get',
        query,
      }),
    post: <Data>(uri: string, body?: Record<string, unknown> | URLSearchParams | FormData, query?: Record<string, unknown>) => $fetch<DataResult<Data>>(
      prefix + uri,
      {
        method: 'post',
        query,
